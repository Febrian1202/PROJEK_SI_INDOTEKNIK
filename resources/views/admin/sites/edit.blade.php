    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-2xl">
        <form action="{{ route('admin.sites.update', $site->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="nama_site" value="{{ $site->nama_site }}" class="w-full rounded-lg border-gray-300 focus:ring-brand-blue" required>
            </div>

            <div class="mb-8">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Lokasi Fisik</label>
                <input type="text" name="lokasi_fisik" value="{{ $site->lokasi_fisik }}" class="w-full rounded-lg border-gray-300 focus:ring-brand-blue" required>
            </div>
