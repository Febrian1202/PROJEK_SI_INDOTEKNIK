<x-admin-layout title="Manajemen Pelamar">
    <div class="flex justify-between items-center mb-6">
        ...
        <form method="GET" action="{{ route('admin.lamaran.index') }}" class="flex gap-2">
            <select name="status" class="rounded-lg border-gray-300 text-sm focus:ring-brand-blue"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($lamaran as $item)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 rounded-full bg-gray-200 overflow-hidden flex-shrink-0">
                                    @if ($item->kandidat->foto)
                                        <img src="{{ asset('storage/' . $item->kandidat->foto) }}"
                                            class="w-full h-full object-cover">
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center text-gray-500 font-bold text-xs">
                                            {{ substr($item->kandidat->nama_lengkap, 0, 1) }}
                                        </div>
                                    @endif
                                </div>
                        ...
                        <td class="px-6 py-4 text-gray-500">
                            {{ \Carbon\Carbon::parse($item->tgl_lamar)->format('d M Y') }}</td>
                        <td class="px-6 py-4">
                            @php
                        ...
                        <td class="px-6 py-4 text-center">
                            <a href="{{ route('admin.lamaran.show', $item->id) }}"
                                class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-brand-blue/10 text-brand-blue hover:bg-brand-blue hover:text-white transition">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-8 text-gray-400">Tidak ada data pelamar.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
